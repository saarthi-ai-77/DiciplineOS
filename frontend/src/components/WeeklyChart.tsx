        ...
        {last7Days.map((log, index) => {
          ...
          return (
            <div key={index} className="flex-1 flex flex-col items-center gap-1">
              <div className="flex gap-1 items-end h-28 w-full justify-center relative">
                <div
                  className="w-3 bg-accent transition-all z-10"
                  style={{ height: `${buildHeight}%`, minHeight: log?.build_hours ? "4px" : "0" }}
                  title={`Build: ${log?.build_hours || 0}h`}
                />
                <div
                  className="w-3 bg-foreground/60 transition-all z-10"
